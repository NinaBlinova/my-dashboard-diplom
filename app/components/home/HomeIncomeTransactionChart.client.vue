...
import { VisXYContainer, VisScatter, VisLine, VisAxis, VisTooltip, VisCrosshair } from '@unovis/vue'

type ApiItem = { Year: number, Month: number, Income: number, Tax: number, Transactions: number }
type ChartPoint = { x: number, y: number, year: number, label: string }
...
const compact = computed(() =>
  new Intl.NumberFormat(locale.value === 'ru' ? 'ru' : 'en', { notation: 'compact', maximumFractionDigits: 1 })
)
...
const formatCurrency = (v: number) =>
  v.toLocaleString(locale.value === 'ru' ? 'ru-RU' : 'en-US', {
    style: 'currency',
    currency: 'RUB',
...
${d.label}
${t('home.charts.transactions')}: ${d.x.toLocaleString()}
${t('home.charts.income')}: ${formatCurrency(d.y)}
`

const containerData = computed(() => series.value.flatMap(s => s.data))
</script>
...
<template>
  <UCard :ui="{ body: '!px-0 !pt-0 !pb-3' }">
    ...
    <VisXYContainer v-if="containerData.length" :data="containerData" class="h-80">
      ...
      <VisAxis type="x" :tick-format="(v:number) => compact.format(v)" />
      <VisAxis type="y" :tick-format="(v:number) => compact.format(v)" />
